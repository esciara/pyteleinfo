...
def encode(info_groups: dict) -> str:
    encoded_info_groups = ''.join(
        [encode_info_group(info_group[LABEL], info_group[DATA]) for info_group in
         info_groups])
    return ''.join(['{}'.format(STX),
                    encoded_info_groups,
                    '{}'.format(ETX)])
...
def decode(frame, verify_well_formed: bool = True) -> dict:
    ...
    :param frame: list
    :param verify_well_formed:
    :return: a json dict of (label, data) key/value pair
    """
...
def encode_info_group(label: str, data: str, sep: str = SP) -> str:
    encoded_info_group = "{label}{SEP}{data}{SEP}".format(label=label, SEP=sep,
                                                          data=data)
    checksum = _checksum_method_1(encoded_info_group)
    return "{LF}{encoded_info_group}{checksum}{CR}".format(LF=LF,
                                                           encoded_info_group=encoded_info_group,
                                                           checksum=checksum, CR=CR)


def decode_info_group(encoded_info_group: str, verify_well_formed: bool = True,
                      verify_checksum: bool = True) -> (str, str):
    """

    :param encoded_info_group: str
    :param verify_well_formed: Boolean
    ...
    # Extracting checksum
    label_data_and_separators, checksum = encoded_info_group_trimmed[:-1], \
                                          encoded_info_group_trimmed[-1:]
    # TODO: verify checksum and throw exception if not correct
    if verify_checksum:
...
def _verify_frame_well_formed(frame: str):
    """
    Etape B : effectuer le stockage de la totalité de la trame avec vérification de sa conformité protocolaire
    (caractère ASCII « STX » – présence d’un corps d’au moins un groupe – caractère ASCII « ETX »
    (ou caractère ASCII « EOT » en cas d'interruption).
    ...
    if len(beginnings) != len(ends):
        error = "Should have as many beginnings (LF) as ends (CR) to delimit info groups, but had ({}, {})".format(
            len(beginnings), len(ends))
        errors = _append_error(errors, error)
    else:
        ...
        if any(results):
            ...
            error = "Should always have a LF followed by a CR to delimit " \
...
def _extract_label_and_data(label_data_and_separators: str) -> (str, str):
    ...
    #
    #  "[..] le champ « étiquette » ne contient aucun caractère ayant une valeur égale à celle du
    #  caractère-séparateur utilisé pour la trame (caractère ASCII « espace » ou caractère ASCII
    #  « tabulation horizontale » suivant le cas). Par contre, le champ « donnée » contenant l’information
    #  fournie par le groupe peut, lui, contenir des caractères ayant une valeur égale à celle du
    #  caractère-séparateur utilisé pour la trame."
    #
    # Extracting last separator
    reminder_without_last_sep, sep = label_data_and_separators[:-1], \
    ...
    # If more than 1 item in the data part, then there was a separator in the data field
    # In this case, reconstitute the data field
    if len(data_part) > 1:
...
def _verify_checksum(label_data_and_separators: str, checksum: str):
    ...
    if checksum not in (checksum_1, checksum_2):
        raise ChecksumError(label_data_and_separators, checksum, checksum_1,
                            checksum_2)
